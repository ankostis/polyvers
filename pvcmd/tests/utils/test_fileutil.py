...
from polyvers.utils import fileutil
import os
...
def test_ensure_ext(inp, exp):
    got = fileutil.ensure_file_ext(*inp)
    assert got == exp
...
def test_ensure_ext_regex():
    ...
    exp = 'foo.xlt'
    got = fileutil.ensure_file_ext(*inp, is_regex=True)
    assert got == exp


def test_git_repo(ok_repo, no_repo):
    ok_repo.chdir()
    got = fileutil.find_git_root()
...
def test_is_base_or_same(fileset, basepath, longpath, exp, exp_strict):
    fileset.chdir()
    assert fileutil._is_base_or_same(Path(basepath), Path(longpath)) is exp
    assert fileutil._is_base_or_same(Path(basepath), Path(longpath),
...
def test_is_same_file(fileset, f1, f2, exp):
    fileset.chdir()
    assert fileutil._is_same_file(Path(f1), Path(f2)) is exp
