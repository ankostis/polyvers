...
- :func:`populate_vtags_history()` that populates *vtags* on the given
  project instances; certain vtag-related Project methods would fail if
...
def _replace_vtags_in_projects(
    ...
    cloned_projects = [proj.replace(
        _vtags_collected=vtags_by_pname[proj.pname]) for proj in projects]

    return cloned_projects


def populate_vtags_history(*projects: pvproject.Project,
    ...
    :param projects:
        the projects to search *vtags* for
    ...
    .. Note::
       Internally, *vtags* are populated in :attr:`_vtags_collected` which
       by default it is ``None``.  After this call, it will be a (possibly empty)
       list.  Any pre-existing *vtags* are removed from all projects
       before collecting them anew.

    .. Tip::
       To collect all *vtags* OR *vtags* only, use pre-defined projects
...
def assign_tags_to_projects(tags: Sequence[str],
    ...
    for vtag in tags:
        ...
        #
        for proj in projects:
            version = proj.version_from_vtag(vtag)
            if version:
                proj._vtags_collected.append(vtag)
                break
