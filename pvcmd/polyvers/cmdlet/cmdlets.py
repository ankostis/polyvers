...
class TraitSelector():
    """
    Elaborate trait-selection rules of a :class:`HasTrait` class:

    :ivar subclass_selector:
    ...
    3. select all traits owned by classes contained in revese `mro()` order
       from the 1st baseclass inheriting :attr:`class_selector`  and uppwards.

       And if no traits found, ...

    4. don't select any traits.
    """
    def __init__(self,  # type: ignore # return type (BUG?)
                 subclass_selector: type,
                 classprop_selector: str = None,
                 tag_selector: str = None):
        assert subclass_selector
        self.subclass_selector = subclass_selector
        sbcname = subclass_selector.__name__.lower()
        self.classprop_selector = classprop_selector or '%s_traits' % sbcname
        self.tag_selector = tag_selector or sbcname

    def _find_1st_mro_with_classprop(self, has_traits: trt.HasTraits
                                     ) -> Optional[Tuple[trt.MetaHasTraits, Any]]:
        classprop = self.classprop_selector
        for cls in type(has_traits).mro():
            ptraits = vars(cls).get(classprop)
            if ptraits is not None:
                return cls, ptraits  # type: ignore

    def _select_traits_from_classprop(self,
                                      has_traits: trt.HasTraits,
        ...
        if bads:
            raise AssertionError(
                ...
                (first_mro_class.__name__,
                 self.classprop_selector, ', '.join(bads)))

        return tnames

    def select_traits(self, has_traits: trt.HasTraits
                      ) -> Optional[Sequence[str]]:
        if getattr(has_traits, self.classprop_selector, None) == '*':
            return has_traits.traits()

        res = self._find_1st_mro_with_classprop(has_traits)
        if res:
            return self._select_traits_from_classprop(has_traits, *res)

        tnames = has_traits.traits(printable=True)

        if not tnames:
            ## rule 3: Select all traits for subclasses
            #  after(above) `subclass_selector` in mro().
            #
...
class Printable(metaclass=trt.MetaHasTraits):
    ...
    def __new__(cls, *args, **kwargs):  # @NoSelf
        Printable._trait_selector = TraitSelector(Printable)
        return super().__new__(cls)

    def __str__(self):
        tnames_to_print = self._trait_selector.select_traits(self)
