...
pp_tag_format = '{pname}-{vprefix}{version}'
#: Like :data:`pp_tag_format` but for *mono-project* version-tags.
mp_tag_format = '{vprefix}{version}'

#: The default regex pattern breaking :term:`poly-project` version-tags
#: and/or ``git-describe`` output (eg ``projname-v1.9.0a1+18.g887986c6-dirty``)
...
pp_gitdesc_repat = r"""(?xmi)
    ...
    $
"""
#: Similar to :data:`pp_tag_format` but for :term:`mono-project` version-tags
#: like ``v1.9.0a1+18.g887986c6-dirty``.
mp_gitdesc_repat = r"""(?xmi)
    ^
    (?P<pname>)
    {vprefix}
    (?P<version>\d[^-]*)
...
def polyversion(**kw):
    ...
    :param bool mono_project:
      - false: (default) :term:`poly-project`, ie multiple sub-projects per git-repo.
        Tags formatted by *vtags* :data:`tag_format` & :data:`pp_gitdesc_repat`
        (like ``pname-v1.2.3``).
      - true: :term:`mono-project`, ie only one project in git-repo
        Tags formatted as *vtags* :data:`mp_tag_format` & :data:`mp_gitdesc_repat`.
        (like ``v1.2.3``).
    :param str tag_format:
        ...
        - See :data:`tag_format` & :data:`mp_tag_format`
    :param regex gitdesc_repat:
        ...
        - It overrides `mono_project` arg.
        - See :pep:`0426` for project-name characters and format.
        - See :data:`pp_gitdesc_repat` & :data:`mp_gitdesc_repat`
    :param str vprefixes:
    ...
    .. Note::
       ...
       function with elaborate error-handling :func:`polyvers.gitag.describe_project()`
    ...
    if tag_format is None:
        tag_format = mp_tag_format if mono_project else pp_tag_format
    if gitdesc_repat is None:
        gitdesc_repat = mp_gitdesc_repat if mono_project else pp_gitdesc_repat
